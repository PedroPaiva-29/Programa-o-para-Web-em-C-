...
namespace MBGestaoEscolarAN.Services.Implementations
{
    public class CoordenadorService:ICoordenadorService
    {
        ...
        {
            return await _contexto.Coordenadores.AsNoTracking().FirstOrDefaultAsync(x => x.CoordenadorId == id);
        }

        public async Task<IEnumerable<Coordenador>> ListarTodosASync()
        {
            return await _contexto.Coordenadores.AsNoTracking().OrderBy(x=>x.CoordenadorId).ToListAsync();
        }
    }
}
