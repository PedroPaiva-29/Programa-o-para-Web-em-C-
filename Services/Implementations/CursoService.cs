...
namespace MBGestaoEscolarAN.Services.Implementations
{
    public class CursoService:ICursoService
    {
        ...
        {
            return await _contexto.Cursos.AsNoTracking().FirstOrDefaultAsync(x => x.CursoId == id);
        }

        public async Task<IEnumerable<Curso>> ListarTodosASync()
        {
            return await _contexto.Cursos.AsNoTracking().OrderBy(x => x.CursoId).ToListAsync();
        }
    }
}
